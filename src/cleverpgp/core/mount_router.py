...
from cleverpgp.core.block_volume import BlockVolumeError, EncryptedBlockVolume
from cleverpgp.core.errors import InvalidContainerError, MountUnavailableError
...
class AutomaticMountManager:
    ...
    def create_and_mount_isolated(
        ...
        **options: object,
        ...
        """Create the Windows disk in a process isolated from the Qt runtime."""
        ...
        manager = self._system_manager
        creator = (
            getattr(manager, "create_and_mount_isolated", None)
            if manager is not None
            else None
        )
        if not callable(creator):
            raise MountUnavailableError(
                "Изолированный компонент создания диска недоступен."
            )
        mounted = creator(source, master_key, **options)
        self._active_manager = manager
        self._mounted_container = source
    ...
    def create_hidden_and_mount_isolated(
        ...
        **options: object,
        ...
        """Create a hidden Windows disk in a process isolated from Qt."""
        ...
        manager = self._system_manager
        creator = (
            getattr(manager, "create_hidden_and_mount_isolated", None)
            if manager is not None
            else None
        )
        if not callable(creator):
            raise MountUnavailableError(
                "Изолированный компонент скрытого диска недоступен."
            )
        mounted = creator(source, outer_password, hidden_password, **options)
        self._active_manager = manager
        self._mounted_container = source
        return str(mounted)
