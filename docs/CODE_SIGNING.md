...
Для публичных Windows-выпусков нужен доверенный OV Code Signing Certificate,
выданный физическому лицу. Закреплённое имя издателя:
...
1. Установить программное обеспечение аппаратного токена, предоставленное центром
   сертификации.
2. Подключить токен и убедиться, что сертификат появился в личном хранилище
   сертификатов Windows текущего пользователя или компьютера.
...
PIN токена не записывается в переменные, файлы или историю команд. Его следует
вводить только в защищённом окне программного обеспечения токена при подписи.

## Подписанная сборка

Для сертификата Sectigo достаточно указать отпечаток:
...
официальный сервер метки времени Sectigo:
...
```powershell
Get-AuthenticodeSignature "E:\Clever\_PGP\Clever-PGP-Setup-0.11.0.exe" |
